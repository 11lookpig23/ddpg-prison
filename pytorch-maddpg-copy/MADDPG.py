...
class MADDPG:
    def __init__(self, n_agents, dim_obs, dim_act, batch_size,
        ...
        if ifload:
            for i in range(2):
                name1 = "parameter/actor_v1"+str(i)+".pth"
                name2 = "parameter/critic_v1"+str(i)+".pth"
                print(name1)
                self.actors[i].load_state_dict(th.load(name1))
                self.critics[i].load_state_dict(th.load(name2))
        ...
        if self.steps_done % 30 == 0 and self.steps_done > 0:
            for i in range(self.n_agents):
                ...
                if self.steps_done % 600 == 0: 
                    th.save(self.critics[i].state_dict(),"parameter/critic_v1"+str(i)+".pth")
                    th.save(self.actors[i].state_dict(),"parameter/actor_v1"+str(i)+".pth")
